...
>
> **A related, bigger-scope version of the sell-price fix above was found but deliberately not applied yet.** `generate_live_strategies.py`'s `save_shadow_state()` writes each of the four shadow strategies' (Balanced/Conservative/Reactive/Differential) `selling_price` straight from `choice["squad"]["value"]` — the live *buy* price, not a real sell value — every single run, for every strategy, not just `real_team`. This is the exact same approximation `real_team` had (and the profit-share fix corrected there), just never noticed here because these are shadow squads with no real budget to visibly overflow the way the real FPL app did. `strategies.py`'s docstring claim that every strategy shares "the same sell-value rules" as the backtest engine was flatly wrong — fixed to stop claiming that. The actual fix (tracking each shadow pick's real buy price across gameweeks, the way `live_state_real_team.json` now does, and computing `selling_price` via `sell_value()` each run) touches bank/budget for all four strategies at once and would need the same care the `real_team` fix got — verified against real data, not applied speculatively mid-review. Left as a flagged, scoped-out finding rather than a same-session fix.
