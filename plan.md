...
>
> **The GW3/GW4/GW5 pattern above — a hand-rewritten scratch script every single gameweek — got consolidated into `model/generate_real_team.py`.** Same training/scoring/recommendation flow, just committed once instead of re-derived from scratch each week (three separate copies existed before this). Running the new script for real immediately caught one more instance of the exact bug the budget fix above was supposed to close: the *recommendation's displayed* bank still summed live `now_cost` for the new squad, even though `choose_team()`'s own internal budget was already correct (real sell prices went in via `live_state_real_team.json`) — so a squad that genuinely fit inside budget displayed as £0.1m over. Fixed by reading `squad_bank()` (already computes this correctly off `choice["budget"]`) instead of re-deriving spend from scratch. A reminder that "fixed the budget math" and "fixed everywhere the budget math gets displayed" aren't the same claim.
...
> - **Season records** — highest single-gameweek score any manager has posted (and which gameweek), the season's top points scorer, the most-owned player, the most-transferred-in player, and the most bonus points earned. All of this turned out to already be cumulative in `bootstrap-static`'s `elements[]` — no per-gameweek summing needed, one fetch has the whole season's aggregate. A sixth record, most-captained, was added but deliberately *not* claimed as a season total — FPL only ever reports `most_captained` per gameweek, with no public per-manager pick history to sum across weeks from, so it's labelled with its actual gameweek rather than implying a full-season figure the data can't support.
>
...
> [!CAUTION]
> **`real_team`'s GW1-5 history was wrong, and not by a small margin — corrected against 5 real screenshots of the actual FPL app.** Real scores: GW1=31 (tracked as 33), GW2=76 (79), GW3=36 (32), GW4=73 (69), GW5=65 (55) — season total after GW5 is 281, not 268. Every gameweek's raw player points (captain doubling backed out) had to sum to exactly the screenshot's own displayed Total Pts before anything got written; all 5 matched on the corrected verification pass (see below). Screenshots kept as source evidence in `data/real_team_screenshots/`.
>
> **Found a real player-identity bug in the process, not just a scoring drift.** The bench slot shown every week as "Elliott" was Harvey Elliott (Liverpool, element 383, red kit) — this project had been tracking Elliot Anderson (Man City, element 481) under that name since `real_team`'s GW1 squad was first built as a copy of Balanced's, and never caught the mismatch because both are plausible-looking "Elliot[t]" names and nothing before this cross-checked kit colour against the stored identity. GW5's real lineup also had two further corrections found only by comparing against the screenshot directly: Guéhi was actually benched that week (not starting, as previously tracked), and Martin Ødegaard was a real squad member the whole time that had never been tracked at all.
>
> **The verification math itself had a bug on the first attempt, caught by its own assertion before writing anything.** Summed each player's stored (raw, undoubled) points against the screenshot's displayed total and got GW1 off by exactly 2 — the captain's contribution needs counting twice for that specific check (once for their raw points already in the sum, once more for the double), even though what actually gets *stored* per player is deliberately the undoubled raw value (matching how the other five strategies already persist captain points, multiplier applied only in the aggregate). Fixed the check, not the data model; all 5 gameweeks then verified cleanly.
>
...
>
> **A hits correction that was itself wrong, caught only once the user checked the real season total against the official PL app.** GW3-5 were first recorded with an *estimated* hit count (2/1/1), inferred from standard free-transfer accrual given the real transfer counts (0, 0, 4, 2, 2). That was then reverted to 0 on the theory that the pitch-view screen's "Total Pts" tile must be net-of-hits, since summing each gameweek's displayed player points matched that tile exactly with no 4-point gap — reasoning that felt like checking the arithmetic, but rested on an unverified assumption about what that specific tile actually shows. It was wrong: the user's real season total is **265**, not the 281 that zeroing the hits produced. `281 - 265 = 16`, exactly what the *original* 2/1/1 estimate predicts (8+4+4) — the pitch-view tile shows gross points, hits are only ever reflected in the official running total, which this particular screen never displayed at all. Restored hits to 2/1/1 and recomputed each `gw_score` as raw minus `4 * hits`: GW3 36→28, GW4 73→69, GW5 65→61, cumulative 265 — independently confirmed against the real app, not re-derived from the same ambiguous screen that caused the wrong correction in the first place. Two lessons, not one: (1) "the numbers reconcile" only proves the arithmetic is internally consistent, not that the *assumption behind* the arithmetic is correct — the gross-sum matching the gross-total was never evidence about hits either way; (2) when a plausible inference and a user's real-world number disagree, the real number wins, full stop, no re-litigating the inference.
>
...
> [!CAUTION]
> **A full code review turned up a real, significant bug: 4 of the 5 live strategies had been silently ignoring their own `transfer_margin`/`max_hits_per_gw` config since the live dashboard was first built.** `choose_team()`/`plan_transfers()` in `live_pipeline.py` hardcoded the module-level `TRANSFER_MARGIN`/`MAX_AUTOMATED_HITS` constants (Balanced's own tuned values) for every caller — `strategies.py`'s per-strategy config was never threaded through to the live path at all, only to the backtest path (`simulate_season.py`, which always took these as real parameters). Concretely: **Conservative's `max_hits_per_gw=0` ("never takes a hit") was never actually enforced live** — it ran with the same hit ceiling as Balanced. Since Conservative's `transfer_margin`, `lookahead_gws`, and `differential_weight` are all otherwise identical to Balanced's too, this made it functionally indistinguishable from Balanced in the live pipeline, end to end — which matches the real data exactly: Balanced and Conservative's GW1-4 scores were identical on every single gameweek. Differential's `transfer_margin=0.5` was similarly never applied (always ran at 1.5) — only its ownership tilt was correctly wired in. Fixed by adding `transfer_margin`/`max_hits` as optional parameters, defaulting to the same constants so `live_pipeline.py`'s own real-account submission path is unaffected (Balanced's tuned values *are* those defaults), and updating `generate_live_strategies.py`'s call site to pass each strategy's real config. Verified with a spy on `select_squad()`: `max_hits=0` now tries zero hit-taking candidates at all. This changes what Conservative/Differential actually recommend starting with the next dashboard refresh — flagging clearly since it's a real behavior change to what's shown live, not a cosmetic fix.
>
> **Same review also found the deadline reminder email has been showing the wrong squad since GW2.** `send_deadline_reminder.py`'s `build_email_html()` received the target gameweek number but never used it — always read `squad["gameweeks"][0]`, correct back when that array held a single entry, silently wrong the moment `generate_live_strategies.py` started accumulating real history there. Every 2d/1d/12h/1h reminder since GW2 has shown GW1's squad regardless of which gameweek was actually due. Same root cause as the website's leaderboard/default-view bug fixed weeks earlier — that pass just never touched this script. Fixed to look up the entry matching the target `gw`; verified GW5's email now renders different players than GW1's.
>
> **And a smaller one, in `optimizer.py`: `VALID_FORMATIONS` (and `FantasyRules.md`'s own formation list) enumerated only 7 of the 8 formations FPL's real rule actually permits** — 5-2-3 (5 DEF/2 MID/3 FWD) is legal under "≥3 DEF, ≥2 MID, ≥1 FWD, 10 outfield" but was missing from both the code and the docs describing it, so `pick_starting_xi()` would never consider it even when it would score more than any of the other 7. `apply_auto_subs()` elsewhere in this codebase already checks legality via that general rule rather than a fixed list, so auto-subs could already legally land on 5-2-3 — only the initial XI-picker's hardcoded enumeration disagreed with the rest of the codebase. Added it; purely additive to the search space, so this can only ever match or improve a chosen XI's score, never worsen one.
>
> Two threads chased and ruled out, for the record: `multi_season_backtest.py` calling `simulate()` without pre-building the 5-model ensemble looked like it might skip the ensemble for GW1/Wildcard decisions the way the live-path bug above did — but `simulate()` builds it internally when not passed one (`if ensemble_models is None: ...`), so this is a real no-op, not a bug. And README's multi-season table (2055/2193/2049) vs. plan.md's own older copy of the same table (2040/2118/2058, written at an earlier checkpoint) looked like a live discrepancy at first — `data/multi_season_backtest_results.csv` confirms README matches the last actual run of the script exactly; plan.md's copy is simply a dated log entry from before that run, which is how this file is meant to work, not something to silently overwrite.
>
> **None of this was re-validated with a fresh multi-season backtest** — the `transfer_margin`/`max_hits`/formation fixes only affect the *live* dashboard path and the starting-XI search space respectively, not the backtest engine itself (which already took these correctly), so the existing backtest numbers above are unaffected and don't need re-running. Re-running `multi_season_backtest.py` regardless (3 full-season simulations, each a real training + simulate cost) would still be worth doing at some point since it's stale relative to the current single-season checkpoint, but that's a deliberate choice to make, not something to do automatically alongside a code review.
>
> **A related, bigger-scope version of the sell-price fix above was found — and, on reflection, applied the same evening.** `generate_live_strategies.py`'s `save_shadow_state()` wrote each of the four shadow strategies' (Balanced/Conservative/Reactive/Differential) `selling_price` straight from `choice["squad"]["value"]` — the live *buy* price, not a real sell value — every single run, for every strategy, not just `real_team`. Exact same approximation `real_team` had, just never noticed here because these are shadow squads with no real budget to visibly overflow the way the real FPL app did. `strategies.py`'s docstring claim that every strategy shares "the same sell-value rules" as the backtest engine was flatly wrong — fixed to stop claiming that. First instinct was to flag this and stop, on the reasoning that touching four strategies' budget math without the same verification the `real_team` fix got would be reckless mid-review — but shadow squads carry no real transaction risk the way `real_team`'s did (there's no FPL app that can reject an over-budget shadow squad), which is a materially different risk profile from the fix that motivated the caution in the first place. Applied it: `save_shadow_state()` now tracks each pick's real `buy_price` across gameweeks (retained picks carry theirs forward, new buys get today's value) and computes `selling_price` via `sell_value()`, same mechanism `real_team` already used. Verified with a synthetic GW1→GW2 price-rise scenario (retained pick's buy_price carries forward unchanged, selling_price reflects half the real profit rather than the full current value) and the migration fallback for pre-fix saved state lacking `buy_price` entirely, before touching real data.
