...
>
> **There's a sixth, non-model "strategy": Live Updated Team**, tracking the real squad actually held in the FPL app gameweek by gameweek — including the gameweeks where the plan and reality diverged (a missed transfer, a different pick than recommended). It's ground truth, not optimizer output, so it's kept off the public dashboard and shown on the admin page only (`?` footer link → "See admin page").
>
> **The dashboard also shows real public benchmarks**, not just each strategy compared to itself: the real cumulative average-manager score as a leaderboard row, and a Season records card (highest single-gameweek score, top points scorer, most-owned player, most-transferred-in player, most bonus points, most captained) — all from one free, unauthenticated `bootstrap-static` fetch ([`model/generate_benchmarks.py`](model/generate_benchmarks.py)), no manager ID needed.

> [!CAUTION]
> Every finished 2026-27 gameweek is locked in as real, scored history the moment its results are final — each strategy's actual result feeds a running season total, and the next gameweek is planned from that real squad via normal transfer rules, not rebuilt from scratch each time (see `plan.md`). The 2025-26 numbers below are still the trustworthy ones for judging the model itself: full-season backtests where it never saw the result it was predicting. A handful of real gameweeks is still far too small a sample to draw conclusions from.
>
...
| [`model/fetch_availability_data.py`](model/fetch_availability_data.py) | Fetches real, point-in-time injury/availability data per past gameweek from the [Randdalf/fplcache](https://github.com/Randdalf/fplcache) archive — the historical results dataset only records what *happened*, not what was known before each deadline. Tried as a model feature and reverted (see `plan.md` Phase 4); the fetcher and data are kept as a foundation. |
...
| [`model/price_model.py`](model/price_model.py) | Predicts next-gameweek price moves from net transfer traffic. Deliberately not wired into squad selection itself — only breaks ties between options the points model rates as equal, since letting price drive selection outright would amplify the budget path-dependency instability documented in `plan.md` Phase 4. |
...
| [`model/generate_live_xg_strategy.py`](model/generate_live_xg_strategy.py) | Live counterpart to `run_xg_strategy.py` — Balanced's policy with xG/xA features on, against the live 2026-27 API. Kept as its own process for the same feature-column-mutation reason as the backtest version. |
| [`model/generate_real_team.py`](model/generate_real_team.py) | Generates the **Live Updated Team** recommendation (see the note above) — the real squad actually held, tracked separately from the five model strategies and shown on the admin page only. Reads real buy prices from `data/live_state_real_team.json` for an accurate transfer budget; never writes that file's picks itself, since only a human can say what was actually done. |
| [`model/generate_benchmarks.py`](model/generate_benchmarks.py) | Public benchmarks from one free, unauthenticated `bootstrap-static` fetch — the real cumulative average-manager score, highest single-gameweek score this season, top points scorer, most-owned player, most-transferred-in player, most bonus points, and the latest gameweek's most-captained player. No manager ID or login needed. Shown on the dashboard as a leaderboard row and a Season records card. |
...
| [`model/send_deadline_reminder.py`](model/send_deadline_reminder.py) | Emails the Balanced squad + deadline at 2d/1d/12h/1h before each gameweek deadline ([`.github/workflows/deadline-reminder.yml`](.github/workflows/deadline-reminder.yml)), idempotent per window via `data/reminders_sent.json`. Skips a gameweek entirely once marked done in `data/admin_status.json`. |
