    ...
    <section class="info-panel" id="infoPanel" style="display:none">
      ...
      <div class="info-card glass">
        ...
        <div class="info-grid">
          ...
          <div class="info-item"><div><strong>Season records card</strong>Leads with the most recently finished gameweek's own highest score, then the all-time season record for that same stat (can be, and usually is, an earlier gameweek — a real record, not stale data), the season's top points scorer, the most-owned player, the most-transferred-in player, and the most bonus points earned — all pulled straight from FPL's public API, no manager ID or login needed. "Most captained" is labelled with its gameweek since FPL only reports that per gameweek, not as a season total. Live season only: once a season ends, FPL's live API stops serving it, so there's nothing fresh to compute for the 2025-26 backtest.</div></div>
        </div>
      </div>
...
<script>
  ...
  function renderAdminRealTeam(){
    ...
    // Open on the most recent *played* gameweek -- the admin page is for
    // reviewing real results, so landing on an unplayed future gameweek
    // (which correctly reads "Not yet played") looks like the page is stale.
    let lastPlayed = -1;
    gws.forEach((g, i) => { if (g.season_total != null) lastPlayed = i; });
    let idx = lastPlayed >= 0 ? lastPlayed : gws.length - 1;
